...
section('media');
t('one featured media row (Webby honoree, sourced from the story site registry)', (html.match(/class="mrow reveal"/g) || []).length === 1 && /class="mmedia"><video[^>]*product-led-alliance-summit\.mp4/.test(html));
t('no invented venue/keynote framing for the Webby video', !/Product-Led Alliance Summit/i.test(html) && !visibleText.includes('Meta-wins') && !/>Keynote</.test(html));
...
t('three videos: honoree clip, workshop, reel', (html.match(/<video /g) || []).length === 3 && /bts-1\.mp4/.test(html) && /behind-the-work\.mp4/.test(html) && !/bts-3\.mp4/.test(html));
...
t('podcast card: original soundwave art in PCC colors, not a claimed BTS clip',
  /<rect width="400" height="275" fill="#435030"\/>/.test(html) && (html.match(/<line x1="/g) || []).length === 9 && !/bts-3\.mp4/.test(html));
